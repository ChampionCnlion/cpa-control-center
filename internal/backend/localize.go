...
var translations = map[string]map[string]string{
	localeEnglish: {
		...
		"task.scan.recovery_candidates":         "Quota recovery probes: candidates=%d confirming=%d due=%d uninitialized=%d selected=%d selected_confirming=%d selected_due=%d",
		"task.scan.recovery_outcomes":           "Quota recovery results: waiting_confirmation=%d ready_to_reenable=%d threshold_blocked=%d",
		...
		"task.maintain.recovery_waiting_manual": "Waiting for the next maintain run to confirm %d accounts before re-enabling; scheduler is currently disabled",
		...
		"task.quota.bucket_summary":             "Bucket parse for %s (%s): 5h=%s, weekly=%s, review=%s",
		"task.quota.bucket_success":             "success %s%%",
		"task.quota.bucket_success_reset":       "success %s%% @ %s",
		"task.quota.bucket_failed":              "failed",
		"task.quota.bucket_unsupported":         "unsupported",
	...
	localeChinese: {
		...
		"task.scan.prepared_candidates":         "已整理出 %d 个候选账号，来自 %d 条清单记录",
		"task.scan.recovery_candidates":         "额度恢复探测：候选=%d，待二次确认=%d，到期=%d，未初始化=%d，本轮选择=%d，二次确认入选=%d，到期入选=%d",
		"task.scan.recovery_outcomes":           "额度恢复结果：待下一轮二次确认=%d，已恢复待启用=%d，低于恢复阈值继续禁用=%d",
		...
		"task.maintain.recovery_waiting_manual": "有 %d 个账号已通过第一次恢复确认；当前未启用定时维护，需要再次执行维护后才会恢复启用",
		...
		"task.quota.bucket_summary":             "%s（%s）的额度桶识别：5小时=%s，周=%s，代码审查周=%s",
		"task.quota.bucket_success":             "成功 %s%%",
		"task.quota.bucket_success_reset":       "成功 %s%%，重置于 %s",
		"task.quota.bucket_failed":              "失败",
		"task.quota.bucket_unsupported":         "不支持",
