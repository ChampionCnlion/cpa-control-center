...
func TestLimitQuotaRecoveryProbeCandidatesPrefersConfirmingOverDue(t *testing.T) {
	now := time.Now().UTC()
	candidates := []AccountRecord{
		{
			Name:              "confirming.json",
			Disabled:          true,
			ManagedReason:     "quota_disabled",
			StateKey:          stateQuotaLimited,
			RecoveryPassCount: 1,
	...
	if len(selected) != 2 || selected[0].Name != "regular.json" || selected[1].Name != "confirming.json" {
		t.Fatalf("expected regular candidate plus confirming recovery candidate, got %+v", selected)
	}
	if len(indexes) != 2 || indexes[0] != 2 || indexes[1] != 0 {
		t.Fatalf("expected selected indexes [2 0], got %+v", indexes)
	}
}
...
func TestBackendMaintainDoesNotReenableStaleRecoveredWithoutCurrentProbe(t *testing.T) {
	var (
		mu         sync.Mutex
		probeCalls int
		reenabled  []string
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v0/management/auth-files":
			_ = json.NewEncoder(w).Encode(map[string]any{
				"files": []map[string]any{
					{
						"name":       "stale-recovered.json",
						"type":       "codex",
						"provider":   "codex",
						"auth_index": "stale-recovered",
						"disabled":   true,
						"id_token":   `{"chatgpt_account_id":"acct-stale-recovered","plan_type":"free"}`,
					},
				},
			})
		case r.Method == http.MethodPost && r.URL.Path == "/v0/management/api-call":
	...
	if err := service.store.UpsertCurrentAccount(AccountRecord{
		...
		StateKey:             stateRecovered,
		PlanType:             "free",
		Disabled:             true,
		ManagedReason:        "quota_disabled",
		AuthIndex:            "stale-recovered",
		ChatGPTAccountID:     "acct-stale-recovered",
		LastSeenAt:           nowISO(),
		LastProbedAt:         nowISO(),
		UpdatedAt:            nowISO(),
		RecoveryPassCount:    2,
		RecoveryLastPassedAt: nowISO(),
		RecoveryNextProbeAt:  now.Add(time.Hour).Format(time.RFC3339),
	}); err != nil {
		t.Fatalf("UpsertCurrentAccount: %v", err)
	}
	...
	if len(result.ReenableResults) != 0 {
		t.Fatalf("expected no reenable results for stale recovered account, got %+v", result.ReenableResults)
	}
	...
	if !records[0].Disabled {
		t.Fatalf("expected stale recovered account to remain disabled, got %+v", records[0])
	}

	mu.Lock()
	defer mu.Unlock()
	if probeCalls != 0 {
		t.Fatalf("expected no probe call for stale recovered account, got %d", probeCalls)
	}
	if len(reenabled) != 0 {
		t.Fatalf("expected no reenable API call, got %+v", reenabled)
	}
}
